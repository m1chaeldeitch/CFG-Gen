...
def validate_thread_creation2(cfg, all_cfgs, all_func_names):
    thread_create_nodes = []
    thread_create_map = {}
    #thread_create_nodes["func_name"] = [node1, node2] todo Maybe adapt to this, not necessary under current plan
    funcs_called = []
    ...
    for i in range(len(cfg.nodes)):
        ...
        for statement in basic_block._statements:
            if statement is not None:
                statement_search = ast_operations.find_pthread(statement)
                ...
                if statement_search.found is True:
                    thread_create_nodes.append(cfg.nodes[i])
        ...
        if thread_create_nodes:
            thread_create_map[cfg.nodes[0]['label'].split(" ")[1].strip("()")] = thread_create_nodes
    ...
    for func in funcs_called:
        if func in all_func_names:
            ...
            additional_nodes = validate_thread_creation2(curr_cfg, all_cfgs, all_func_names)
            thread_create_nodes.extend(additional_nodes)
